...
| Article XI — ontology is domain truth | `tests/contract/test_ontology.py`, `tests/contract/test_schema.py`, `tests/contract/test_ontology_modules.py` (composition, imports, ownership, collisions), `tests/contract/test_ontology_generate.py` + `tests/contract/test_ontology_release.py` (codegen drift and the six compatibility gates), `tests/contract/test_ontology_change_flow.py`, ontology CI gates (`validate`, `generate --check`, `check-release`) | Implemented through Phase 3: the artifact is a composition with per-name ownership, and four CI gates guard it |
...
| Article XIV — core is domain-neutral | `tests/component/test_core_independence.py` (AST sweep: `aegis` imports `legacy.*` only at the two enumerated ADR-023 exemptions, and never from `projections`/`analytics`), `tests/contract/test_second_domain.py` (vocabulary sweep over every file in `aegis/` and `ui/src`), `tests/integration/test_second_domain.py` + `tests/integration/test_second_domain_routes.py` (the `border-cargo` fixture domain recorded, projected and served by an unmodified core) | Implemented: the import boundary at T21 (H-36), ontology-module neutrality at T31/T40 |
...
| Phase 4 investigation model and object views | `tests/contract/test_phase_04_status.py` (spec 09 covers the operational model and every object-view surface; each P4-tagged finding is dispositioned; the leak-free case list is specified by construction, not by filtering; the as-of promise reads as narrowed; no gate criterion is checked before the exit review) | **Planned in the active phase.** T41 lands the spec and its coverage gate; the executable model, route, object-view and as-of suites arrive with T43–T49 and are listed in spec 09 §8 |
