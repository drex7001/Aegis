...
| `GET /v1/alerts?watchlist=&status=` | analyst | per row | detections are **typed alerts** in `watchlist_alert`, **not** review-queue rows (corrected at T75 — ADR-060 gives the five reasons the queue cannot carry them). Filtered on the alert's own `handling_rank`, derived from the claims that fired it, because the queue filters on the *source record's* code and a `sensitive` claim can sit in an `open` record. Dedupe key `(watchlist_id, rule_version, matched_value, entity_id)` | limit ≤ 50 | `test_watchlists.py`, `test_watchlist_routes.py` |
