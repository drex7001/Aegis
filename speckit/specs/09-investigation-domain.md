...
| Region | Source | Rule |
|---|---|---|
| Title / subtitle | `entity.label`, over the type's descriptor label and id | **Corrected at T44.** This section first said the heading resolves `display.title`/`display.subtitle` against claim-derived properties. It cannot: those name *properties* and the response is keyed by *predicate*, and the ontology declares no mapping between the two — the server's own correspondence (`aegis/authz/filters.py`) is a documented heuristic, not a contract. **P5 update (ADR-047):** the
correspondence is now *declarable* — a literal-object predicate may name the
property it carries, and `property_sensitivity` reads the declaration first.
That makes field sensitivity a statement rather than a name coincidence; it does
not change this row, because the heading still comes from `entity.label`. So the
heading is `entity.label`, which search, the graph and the projection already show and which `hidden_entity_types` already makes clearance-aware; the subtitle is the type's label (linked to its schema page) beside the entity id. `display` still governs the **type** page (T42), which renders a schema rather than an instance, and where the property names are exactly what a reader wants |
