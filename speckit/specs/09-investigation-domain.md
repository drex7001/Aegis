...
| Region | Source | Rule |
|---|---|---|
| Title / subtitle | `display.title` / `display.subtitle` resolved against claim-derived properties | Falls back to the entity id — never to a blank heading |
| Properties | `GET /v1/entities/{id}` → `claims_by_predicate` | Each value carries all three grading dimensions (Article III). **Conflicting values render side by side**, both with their `contradicted_by` badge — two dates of birth are two dates of birth (Article VIII, `conflicts: preserve`). Never a "primary" value with the rest hidden behind a disclosure |
| Links | same response, grouped by `PREDICATES[p].category` | Category label and colour come from `CATEGORIES`. An uncategorized predicate groups under "Other" rather than vanishing |
