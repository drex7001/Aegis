...
- **The rail** (T42) — spec 07 §4's left column, in two groups: the workspace
  views above, and every declared object type and interface below, listed from
  the generated descriptors. The case switcher is not here yet; it arrives with
  T46's case list, because a nav entry with no destination is the promise this
  workspace does not make.
- **Object-type and interface pages** (T42) — `/types/:name` and
  `/interfaces/:name`, showing what the ontology *declares*: properties with
  their labels, types, cardinality, required clearance and conflict policy; the
  interfaces a type implements; and every predicate that can attach to it,
  grouped by category in the ontology's own colours. They call no endpoint.
  Adding a type to a domain module and regenerating produces its page with no
  React change — the schema half of the phase's fourth exit criterion.
- **The ontology-version banner** (T42) — the bundle's `ONTOLOGY_VERSION`
  against the server's, non-blocking, because the server is authoritative for
  every value that matters and only the captions can be stale.

Still to come in P4: object views (entity-360), cases, hypotheses, tasks and the
timeline with as-of.
...
`react-router` since **T42**, when `/types/:name` made a route parameter real.
P2 routed on the History API directly and deferred the library for one specific
reason: it would own the history stack and therefore not observe the sign-in
callback's `replaceState`, which had already rendered the fallback view at the
correct address once.

**That hazard is gone rather than worked around.** `onSigninCallback` no longer
touches `history` — it records `returnTo` in a module variable, and
`auth/SigninCallback.tsx` (the `/auth/callback` route) navigates there with
`replace: true`. One owner of the URL, no event to remember to dispatch, and
`replace` still does the two jobs it always did: the spent authorization code
leaves the address bar, and the callback stays out of the back button.

`src/routing.ts` keeps the **route table** and its path builders
(`objectTypePath`, `interfacePath`), so a link is never a hand-written string
and a renamed route is a type error. `safeReturnTo` is the one guard worth
knowing about: `/auth/callback` is never a return destination, or a visit to it
without a code would sign in and return to itself forever.

The rail's ontology section (`layout/OntologyNav.tsx`) and both descriptor
screens read `src/api/ontology.ts` — generated by `aegis ontology generate`,
drift-gated in CI — and call no endpoint. Do not add a type name to them
(ADR-043); `tests/contract/test_workspace_descriptors.py` sweeps `src/` for
every domain-declared name and fails if one appears.
